...
class RainChar {
    ...
    _updateParticles() {
        this._particles.forEach(particle => {
            if (particle.y > this._size[1]) {
                ...
                particle.speed = 1; // reset speed to default on recycle
            } else {
                particle.y += particle.size * this._charSpacing * particle.speed;
                // Occasionally change speed to a new random value
                if (Math.random() < 0.01) {
                    particle.speed = 0.5 + Math.random() * 1.5; // between 0.5 and 2
                }
            }
        });
    }
    ...
    _getCharCodes() {
        if (this._fixedChars) {
            this._charCodes = this._fixedChars;
        } else if (Array.isArray(this._charRange[0])) {
